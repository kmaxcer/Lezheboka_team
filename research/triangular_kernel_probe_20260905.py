...
def pred(mask,h):
 ...
 for j,qi in enumerate(qix):
  ...
  dd=np.abs(doy[sel]-doy[qi]);dd=np.minimum(dd,366-dd); u=dd/(h*366);w=np.maximum(0,1-u); ok=w>0
  if not ok.any():
   k=np.argmin(dd);out[j]=y[sel[k]]
  else: out[j]=np.sum(w[ok]*y[sel[ok]])/np.sum(w[ok])
 return out
rows=[]
for h in [.05,.1,.2]:
 po=pred(outer,h);pg=pred(actual,h);rm=lambda a,b:float(np.sqrt(np.nanmean((a-b)**2)));print(h,'kernel',rm(yo,po),rm(yg,pg));
 rows.append(dict(h=h,w=1.0,outer_rmse=rm(yo,po),released_rmse=rm(yg,pg),kernel_outer=rm(yo,po),kernel_released=rm(yg,pg)))
pd.DataFrame(rows).to_csv(R/'triangular_kernel_probe_20260905.csv',index=False);(R/'triangular_kernel_probe_20260905_report.md').write_text('# Triangular kernel probe\n\nSame AOI/year temporal Nadaraya-Watson with K=max(0,1-|u|/h), leakage-safe target masking.\n\n'+pd.DataFrame(rows).to_string(index=False)+'\n\nNo candidate materialized; upload not performed.\n',encoding='utf8')
