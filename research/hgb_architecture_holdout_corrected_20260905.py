...
for name,sp in specs:
 t=time.time(); m=HistGradientBoostingRegressor(random_state=42,**sp); m.fit(X,y); p=np.clip(m.predict(qx),-.2,1.1); rr=float(np.sqrt(np.mean((p-truth)**2))); rec.append({'model':name,'rmse':rr,'mae':float(np.mean(abs(p-truth))),'mean':float(p.mean()),'seconds':round(time.time()-t,1)}); print(rec[-1],flush=True)
pd.DataFrame(rec).to_csv(R/'hgb_architecture_holdout_corrected_20260905.csv',index=False); print(rec)
